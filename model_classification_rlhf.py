...
class GPT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, **kwargs):
        """
        Modified forward pass compatible with TRL RewardTrainer

        Args:
            input_ids: Token indices of shape (batch_size, sequence_length)
            attention_mask: Mask to avoid attention on padding tokens (optional)
            **kwargs: Additional arguments for compatibility

        Returns:
            For reward model: dict with 'rewards' key containing scalar rewards
            For LM: logits and optionally loss
        """
        ...
        if self.config.is_reward_model:
            # For reward model, use the last token's representation
            if attention_mask is not None:
                # Use the last non-padded token for each sequence
                # attention_mask is (batch_size, seq_len) with 1s for valid tokens, 0s for padding
                sequence_lengths = attention_mask.sum(dim=1) - 1  # -1 because we want 0-indexed
                sequence_lengths = torch.clamp(sequence_lengths, 0, t - 1)

                # Ensure we have the right dimensions
                batch_indices = torch.arange(b, device=device)

                # Extract the last valid token representation for each sequence
                # x is (batch_size, seq_len, hidden_size)
                last_hidden = x[batch_indices, sequence_lengths]
            else:
                # Use the last token if no attention mask
                last_hidden = x[:, -1, :]
    ...
    @classmethod
    def from_pretrained_for_reward(cls, model_type, **kwargs):
        """
        Create a reward model from a pretrained language model
        """
        # First load the pretrained model normally
        model = cls.from_pretrained(model_type, **kwargs)

        # Convert to reward model
        config = model.config
        config.is_reward_model = True
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        assert all(k in ['dropout', 'num_classes'] for k in override_args)
...
class RewardModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, **kwargs):
        ...
        # Return just the rewards tensor - TRL handles this correctly
...
def create_reward_model_from_pretrained(model_type="gpt2", **kwargs):
    """
    Convenience function to create a reward model
    """
    config_overrides = kwargs.copy()
    config_overrides['is_reward_model'] = True

    # Load base model first
    base_model = GPT.from_pretrained(model_type, **kwargs)

    # Create reward version
    reward_config = GPTConfig(
        ...
        **{k: v for k, v in kwargs.items() if k.startswith('lora')}
    )
